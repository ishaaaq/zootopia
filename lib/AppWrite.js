import {
  Client,
  Account,
  Avatars,
  Databases,
  Query,
  Storage,
} from "react-native-appwrite";

import uploadFile from "@/lib/supabase";

export const config = {
  platform: "com.deint.zootopia",
  endpoint: "https://cloud.appwrite.io/v1",
  project: "677ed3a8002febffb9ee",
  database: "67810fc40025cb950ad1",
  zoo: "6781102400312fda053b",
  supplier: "67811c670007b10ef3ad",
  buyer: "67811edf001cb07f401a",
  animal: "6782a44100047a4b7480",
  gallery: "67864581002364639072",
  conversation: "678a01b200075a15c676",
  message: "678a3833001e89cc7e63",
  notification: "67915992002c89c41471",
  transaction: "67953a1700185dd190bd",
  reports: "67a40a64000aba347e33",
};

export const client = new Client();

client
  .setEndpoint(config.endpoint)
  .setPlatform(config.platform)
  .setProject(config.project);

export const avatar = new Avatars(client);
export const account = new Account(client);
export const databases = new Databases(client);
export const storage = new Storage(client);

export async function signup(usertype, values) {
  try {
    const user = await account.create(
      "unique()",
      values.email,
      values.password
    );

    switch (usertype) {
      case "zoo":
        await databases.createDocument(config.database, config.zoo, user.$id, {
          zooname: values.zooName,
          location: values.zooLocation,
          regnumber: values.registrationNumber,
          email: values.email,
          phone: values.phoneNumber,
          password: values.password,
          usertype: usertype,
        });
        break;
      case "supplier":
        await databases.createDocument(
          config.database,
          config.supplier,
          user.$id,
          {
            name: values.businessName,
            email: values.email,
            phone: values.phoneNumber,
            password: values.password,
            usertype: usertype,
          }
        );
        break;
      case "buyer":
        await databases.createDocument(
          config.database,
          config.buyer,
          user.$id,
          {
            name: values.name,
            email: values.email,
            phone: values.phoneNumber,
            password: values.password,
            usertype: usertype,
          }
        );
        break;
      default:
        throw new Error("Invalid user type");
    }
    return user;
  } catch (err) {
    console.error("Sign Up Error:", err.message);
    throw new Error(err);
  }
}

export async function login(values) {
  console.log("email:", values.email, "password:", values.password);
  try {
    const response = await account.createEmailPasswordSession(
      values.email,
      values.password
    );
    console.log("Login response:", response);
    return response;
  } catch (err) {
    console.error("Login Error:", err.message, err.response);
    throw new Error(err);
  }
}

export async function logout() {
  try {
    const response = await account.deleteSession("current");
    console.log("logged out");
    return response;
  } catch (err) {
    console.error("Logout Error", err.message);
    throw err;
  }
}

export async function getCurrentUser() {
  try {
    const user = await account.get();
    console.log("Current user response:", user);

    let userDetails;

    try {
      userDetails = await databases.getDocument(
        config.database,
        config.zoo,
        user.$id
      );
    } catch (err) {
      try {
        userDetails = await databases.getDocument(
          config.database,
          config.supplier,
          user.$id
        );
      } catch (err) {
        userDetails = await databases.getDocument(
          config.database,
          config.buyer,
          user.$id
        );
      }
    }

    console.log("Custom user properties:", userDetails);
    const userAvatar = avatar.getInitials(
      userDetails.name || userDetails.zooname
    );

    // return userDetails;
    return {
      ...userDetails,
      avatar: userAvatar.toString(),
    };
  } catch (error) {
    return null;
  }
}

export async function addAnimal(supplierId, photo, values, ischecked) {
  try {
    // const imageUrl = await uploadFile(photo.uri);
    console.log("values", values);
    const completeAnimalData = {
      ...values,
      quantity: Number(values.quantity),
      price: Number(values.price),
      image: values.image,
      supplier: supplierId,
      exoticPet: ischecked,
    };
    const newAnimal = await databases.createDocument(
      config.database,
      config.animal,
      "unique()",
      completeAnimalData
    );
    console.log("Animal successfully added:", newAnimal);
    return newAnimal;
  } catch (error) {
    console.error("Error adding animal:", error.message);
    throw error;
  }
}

export const fetchAnimalsForCurrentUser = async () => {
  try {
    const user = await account.get();
    const userId = user.$id;

    const response = await databases.listDocuments(
      config.database,
      config.animal,
      [Query.equal("supplier", userId)]
    );
    return response.documents;
  } catch (error) {
    return [];
  }
};

export const fetchAllAnimals = async () => {
  try {
    const response = await databases.listDocuments(
      config.database,
      config.animal
    );
    console.log("Fetched animos:", response.documents);
    return response.documents;
  } catch (error) {
    console.log("Error getting all animals:", error);
  }
};

export const updateAnimal = async (animalId, updatedData) => {
  try {
    const response = await databases.updateDocument(
      config.database,
      config.animal,
      animalId,
      updatedData
    );
    return response;
  } catch (error) {
    console.error("Error updating animal:", error.message);
    throw error;
  }
};

export const deleteAnimal = async (animalId) => {
  try {
    const response = await databases.deleteDocument(
      config.database,
      config.animal,
      animalId
    );
    console.log("Animal deleted successfully:", response);
    return response;
  } catch (error) {
    console.error("Error deleting animal:", error.message);
    throw error;
  }
};

export const getConversations = async (userId) => {
  const result = await databases.listDocuments(
    config.database,
    config.conversation,
    [Query.contains("participants", userId)]
  );
  console.log("current user conversations:", result.documents);
  return result.documents;
};

export const sendMessage = async (conversationId, senderId, message) => {
  const res = await databases.createDocument(
    config.database,
    config.message,
    "unique()",
    {
      conversationId,
      senderId,
      message,
      timestamp: new Date().toISOString(),
    }
  );

  const conversation = await databases.getDocument(
    config.database,
    config.conversation,
    conversationId
  );

  const receiverId = conversation.participants.find(
    (participant) => participant !== senderId
  );

  if (receiverId) {
    await createNotification(receiverId, "New Message", message);
  } else {
    console.error("Receiver ID not found in conversation participants.");
  }

  return res;
};

export const getUserDetails = async (userId) => {
  try {
    const collections = {
      zoo: "6781102400312fda053b",
      petSupplier: "67811c670007b10ef3ad",
      petBuyer: "67811edf001cb07f401a",
    };

    for (const [usertype, collectionId] of Object.entries(collections)) {
      try {
        const userDetails = await databases.getDocument(
          config.database,
          collectionId,
          userId
        );
        if (userDetails) {
          const userAvatar = avatar.getInitials(
            userDetails.name || userDetails.zooname
          );
          return {
            ...userDetails,
            usertype,
            avatar: userAvatar.toString(),
          }; // Include the type for reference
        }
      } catch (error) {
        if (error.code !== 404) {
          console.error(`Error fetching user from ${usertype}:`, error.message);
        }
      }
    }

    throw new Error("User not found in any collection");
  } catch (error) {
    console.error(
      `Error fetching user details for ID: ${userId}`,
      error.message
    );
    return { name: "Unknown User" }; // Fallback for unknown users
  }
};

export const createNotification = async (userId, type, content) => {
  try {
    await databases.createDocument(
      config.database,
      config.notification,
      "unique()",
      {
        userId,
        type,
        content,
        isRead: false,
        timestamp: new Date(),
      }
    );
  } catch (error) {
    console.error("Error creating notification:", error.message);
  }
};

export const updateSellerStripeAccount = async (sellerId, stripeAccountId) => {
  try {
    await databases.updateDocument(config.database, config.supplier, sellerId, {
      stripeAccountId,
    });

    console.log("Stripe account ID updated successfully.");
    return true;
  } catch (error) {
    console.error("Failed to update seller Stripe account ID:", error);
    throw error;
  }
};

export const fetchTransactions = async (userId) => {
  try {
    const queries = [
      Query.or([
        Query.equal("buyerId", userId),
        Query.equal("sellerId", userId),
      ]),
    ];
    const response = await databases.listDocuments(
      config.database,
      config.transaction,
      queries
    );

    return response.documents;
  } catch (error) {
    console.error("Error fetching transactions:", error);
  }
};

export const saveTransaction = async (transactionDetails) => {
  try {
    const response = await databases.createDocument(
      config.database,
      config.transaction,
      "unique()",
      { ...transactionDetails }
    );
    return response;
  } catch (error) {
    console.error("error saving transaction", error);
  }
};
