...
export const config = {
  ...
  zooAnimal: "678fd696002a0c2b49cb",
};
...
export async function addZooAnimal(zooId, photo, values) {
  try {
    const imageUrl = await uploadFile(photo.uri);
    const completeAnimalData = {
      ...values,
    ...
    console.log("ZooAnimal successfully added:", newAnimal);
    return newAnimal;
  } catch (error) {
    console.error("Error adding zooanimal:", error.message);
    throw error;
  }
}
...
export const fetchAnimalsForCurrentZoo = async () => {
  try {
    const user = await account.get();
    const userId = user.$id;
    ...
    const response = await databases.listDocuments(
      config.database,
      config.animal,
      [Query.equal("zoo", userId)]
    );
    console.log("animals for zoo: ", response.documents);
    return response.documents;
  } catch (error) {
    console.error("Error fetching animals for current zoo:", error.message);
    return [];
  }
};
